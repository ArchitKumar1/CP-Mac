...
freopen("output.txt", "w", stdout);
    ...
    // cin >> n;
    // VVI arr(n,VI(4,0));
    // forn(i,n)forn(j,4) cin >> arr[i][j];
    // vector<int> perm({1,2,3,4});
    // vector<int> fans;
    // int min_cost = INT_MAX;
    // do{
    //     // trace(perm);
    //     VVI new_arr(n,VI(4,0));
        
    //     forn(i,n)forn(j,4){
    //         new_arr[i][perm[j]-1] = arr[i][j];
    //     }
    ...
    //     forn(i,n)forn(j,4){
    //         if(i>=1){
    //             //trace(new_arr[i-1][j],new_arr[i][j]);
    //             curr_cost += abs(new_arr[i-1][j] - new_arr[i][j]);
    //         }
    //     }
    //     // trace(curr_cost,min_cost);
