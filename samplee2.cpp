...
int daysnonleap[12] = {31,28,31,30,31,30,31,31,30,31,30,31};
...
freopen("output.txt", "w", stdout);
	...
	for(int y = 1;y<N;y++){
		...
		for(int m = 0;m<12;m++){
            if(leap(y) == 1){
                ...
                for(int i = 0;i< daysnonleap[i];i++){
                    if(m == 1 && i == 0 && (day == 5 || day == 6)){
                        year[y] +=1;
                    }
                    day = (day+1)%7;
                }
	...
	// 	if(m2>2) y2++;
	// 	int extrayears = 0;
	// 	extrayears += (y2-y1)/2800;
	// 	int y1new = y1 - (y1/2800)*2800;
	// 	int len = (y2-y1)%2800;
	// 	int fans = 0;
	// 	//trace(y1new);
	// 	for(int i = y1new ;i<=y1new+len;i++){
	// 		//cout << "ok" << endl;
	// 		//trace(i,year[i]);
	// 		fans += (year[i] == 1);
	// 	}
	// 	fans += extrayears*year400sum;
	// 	cout <<fans << endl;
	// }
