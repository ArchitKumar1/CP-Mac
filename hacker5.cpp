...
void solve(){
    ...
    vector<int> fact({1,1,2,6,24,120});
    // for(auto x: all_sets){
    //     trace(x);
    // }
        ...
        for(int j = 0;j<all_sets.size();j++){
            ...
            for(int k = 0;k<all_sets.size();k++){
                ...
                forn(x,4){
                    ...
                    if((i==1 ?0: all_sets[j][x] ) + all_sets[k][x]>K) ok = 0;
                }
                if(ok == 0)continue;
                int tempans = fact[5]; 
                for(int p : all_sets[k]){
                    //trace(fact[p]);
                    tempans/= fact[p];
                }
                if(i == 1){
    ...
    int fans = 0;
    forn(i,all_sets.size()){
        fans += m1[{n,all_sets[i]}];
    }
    cout << fans << endl;
