...
struct comp {
    ...
    comp(const db x_ = 0, const db y_ = 0) {
        x = x_;
        y = y_;
    }
...
void multi(LL *a, LL *b, LL *c, int n, int m) {
    ...
    DFT(C);
    reverse(C + 1, C + N);
    fo(i, 0, N - 1)C[i].x /= N, C[i].y /= N;

    DFT(D);
    reverse(D + 1, D + N);
    fo(i, 0, N - 1)D[i].x /= N, D[i].y /= N;
    fo(i, 0, n + m) {
...
void solvef() {
    ...
    solveg();
    fo(i, 1, n)if (i & 1)G1[i] = g[i], G2[i] = g[i] * i % mo;
...
void solve() {
    ...
    //solvef();
    vector<LL> sum2(k+1,0),fin(k+1,0);
    for (int i = 0; i <= k; i++) {
