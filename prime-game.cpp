...
int powm(int a,int b,int m= mod){int res = 1;while(b){ if(b&1) res =((LL)res*a)%m; a = ((LL)a*a)%m;b >>=1;}return res;}
...
int gcnt(int a,int b,int x,int y){
    return cnt[x][y] - cnt[--a][--b];
}
...
int solve(int a,int b,int x,int y){
    ...
    if(a == x && b== y){
        return dp[a][b][x][y] = g[a][b];
    }
    if(gcnt(a,b,x,y) == 0){
        return dp[a][b][x][y] = 0;
    }
    int res = max(x-a+1,y-b+1);
    for(int k = b;k<y;k++ ){
        res = min(res, solve(a,b,x,k) + solve(a,k+1,x,y));
    }
    for(int k = a;k<x;k++ ){
        res = min(res, solve(a,b,k,y)+ solve(k+1,b,x,y));
    }
