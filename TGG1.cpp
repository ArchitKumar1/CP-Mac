...
int g_fact(int n){
    int q = n/mod;
    int r = n%mod;
    return pow_mod(fact[mod],q,mod) * fact[r] % mod;
}
int g_ifact(int n){
    return pow_mod(g_fact(n),mod-2,mod);
}
int ncr(int n,int r){
    return g_fact(n)* g_ifact(r) %mod * g_ifact(n-r)%mod;
}
int get_distri(int n,int a,int b,int c){
    return g_fact(n)*g_ifact(a)%mod*g_ifact(b)%mod*g_ifact(c)%mod;
}
void __Solve__(){
    n
