...
void build(int s,int e,int index){
    ...
    tree[index].v = tree[2*index].v + tree[2*index+1].v;
}

// void update(int l,int val,int s,int e,int index){
//     if(s > l || e < l) return;
//     if(s == l && e == l ){
//         arr[l] =val;
//         tree[index].v = arr[l];
...
//     }
//     int m = (s+e)>> 1;
//     update(l,val,s,m, 2*index);
//     update(l,val,m+1,e,2*index + 1);
//     tree[index].v = tree[2*index].v + tree[2*index+1].v;
// }

int update(int l,int r,int val,int s,int e,int index){
    if(s > r || e < l ){
        return 0;
    }
    if(s >=l && e<=r){
...
void propogate(int l,int r,int s,int e,int index){
    return;
}
int query(int l,int r,int s,int e,int index){
    if(s>=l && e<=r){
        if(tree[index].lazy >0){
            propogate(l,r,s,e,index);
        }
        return tree[index].v;
    }
    if(s > r || e < l ){
    ...
    int m = (s+e)>> 1;
    return query(l,r,s,m, 2*index) + query(l,r,m+1,e,2*index + 1);
}
