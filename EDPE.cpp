...
int solve(int curr,int weight){
    
    if(curr ==  0 || weight == 0 ) return 0;
    int ans;
    if(get(curr,weight) != -1){
        return get(curr,weight);
    }
    if(weight - w[curr-1]>=0){
        ans = max(solve(curr-1,weight),v[curr-1] + solve(curr-1,weight-w[curr-1]));
    }else{
        ans = solve(curr-1,weight);
    }
    put(curr,weight,ans);
    return ans;
}
